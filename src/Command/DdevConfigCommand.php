...
final class DdevConfigCommand extends BaseCommand
{
    ...
    protected function configure()
    {
        $this
            ->setName('tdk:ddev')
            ->setDescription('Create a ddev configuration')
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (is_executable(trim(shell_exec($test . ' ddev') ?? ''))) {
            ...
            $ddevCommand = 'ddev config --docroot public --database mariadb:10.7 --project-name ' . $ddevProjectName . ' --web-environment-add TYPO3_CONTEXT=Development --project-type typo3 --php-version ' . $phpVersion . ' --create-docroot';
