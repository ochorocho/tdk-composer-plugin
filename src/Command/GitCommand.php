...
EOT
    ...
    protected function setCommitTemplate(): int
    {
        ...
        if ($filePath ?? false) {
            ...
            $file = $this->getIO()->ask('Set TYPO3 commit message template [default: .gitmessage.txt]? ', '.gitmessage.txt');
        }
...
EOF;
    ...
    public function cloneRepository(): int
    {
        ...
        if($this->gitService->repositoryExists()) {
            $this->getIO()->info('Skipping git clone because the repository already exists.');
            return Command::SUCCESS;
        }

        $filesystem = new \Composer\Util\Filesystem();
        $filesystem->removeDirectory(BaseService::CORE_DEV_FOLDER);
